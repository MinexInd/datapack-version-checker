  ...
  while (i < n) {
    if (line[i] === '{') {
      ...
      while (j < n) {
        ...
        if (c === '\\' && inQuote) { escaped = true; j++; continue }
        if (inQuote) {
          if (c === quoteChar) inQuote = false
          j++; continue
        }
        if (c === '"' || c === "'") { inQuote = true; quoteChar = c; j++; continue }
        if (c === '{') depth++
        else if (c === '}') {
          depth--
          if (depth === 0) {
...
function applyT4(line: string, cStart: number, cEnd: number): { line: string; changed: boolean } {
  ...
  return {
    line: line.slice(0, countEntry.kStart) + 'count' + ':' + stripped + line.slice(countEntry.vEnd),
    changed: true,
  }
}
