...
import { FEATURE_RULES } from './knowledge'
import { versionNameToDataVersion } from './version'
import { fetchVersions } from './api'
import { CMD_REWRITES, type CmdRewrite } from './fixer'
...
export async function generatePortingPlan(
  ...
  const applicableRewrites = CMD_REWRITES.filter((rw: CmdRewrite) => {
  ...
  for (const func of funcResources) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (tokens.length === 0) continue
      const root = tokens[0].value.replace(/^\//, '')
      const hasRewrite = applicableRewrites.some((rw: CmdRewrite) => rw.matchRoot === root)
      const hasKnowledgeRule = FEATURE_RULES.some(rule => {
        if (rule.type !== 'command' || rule.match !== root) return false
