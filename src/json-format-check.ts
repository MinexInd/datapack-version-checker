...
import type { StructuralIssue } from './types.js'

// ---------------------------------------------------------------------------
// Version-aware JSON semantic format checks.
// Detects structural / field-name / layout changes across MC versions that
// are NOT covered by registry value checks or mcdoc structural validation.
// ---------------------------------------------------------------------------

// Predicate fields that were renamed in 1.20
...
// Loot table functions that gained a required `type` field
...
function checkDamagePredicateFlags(data: any, rel: string, ver: string): StructuralIssue[] {
  ...
  function walk(obj: any, path: string): void {
    ...
    // Damage predicates are objects with "type": "minecraft:damage" or sit
    // inside "conditions" blocks. We flag any removed boolean in predicate JSON.
...
function checkRecipeResult(data: any, rel: string, ver: string): StructuralIssue[] {
  ...
  function walk(obj: any, path: string): void {
    ...
    // Recipe output/result objects: detect old vs new key name
    if ('item' in obj && !('id' in obj) && typeof (obj as any).item === 'string' && cmpVer(ver, '1.20.5') >= 0) {
      // Heuristic: only flag if this looks like a recipe result
      // (has 'item' + possibly 'count', 'components', 'data')
...
/**
 * Check a parsed JSON data blob for version-specific semantic format issues.
...
/**
 * Read + parse a JSON file and check for semantic format issues.
 * Server-side helper (uses readFileSync).
 */
export function checkJsonFormatFile(
  file: string,
  rel: string,
  versionName: string,
): StructuralIssue[] {
  try {
    ...
    return checkJsonFormatSemantics(data, rel, versionName)
  } catch {
    return []
  }
}
