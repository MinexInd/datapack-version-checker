...
export const PORT_RULES: PortRule[] = [
  ...
  { id: "execute_if_predicate", type: "command", scope: "datapack", match: "execute", description: "/execute if predicate commented out (pre-1.15)", fix: { kind: 'rewrite', pattern: /^\/execute\s+.*\s+(?:if|unless)\s+predicate\s/, replacement: "## FIXED(/execute if predicate requires 1.15+): $0", sourceSince: "1.15", targetUntil: "1.14.4" } },
  { id: "execute_if_function", type: "command", scope: "datapack", match: "execute", description: "/execute if function commented out (pre-1.20)", fix: { kind: 'rewrite', pattern: /^\/execute\s+.*\s+(?:if|unless)\s+function\s/, replacement: "## FIXED(/execute if function requires 1.20+): $0", sourceSince: "1.20", targetUntil: "1.19.4" } },
