...
import { FEATURE_RULES } from './knowledge.js'
import { versionNameToDataVersion } from './version.js'
import { CMD_REWRITES, type CmdRewrite } from './fixer.js'
...
export async function generatePortingPlan(
  packDir: string,
  targetVersion: string,
  sourceVersion?: string,
): Promise<PortingPlan> {
  ...
  if (!targetVer) {
    return {
      ...
      targetVersion,
      portingForward: false,
      actions: [],
      cascadeEffects: [],
      manualAttention: [{ file: '', description: 'Target version not found', reason: `Unknown version: ${targetVersion}` }],
      summary: { totalActions: 0, autoFixable: 0, manualRequired: 1, filesAffected: 0, cascadeCount: 0 },
    }
  }

  let sourceVer: McmetaVersion | null = null
  if (sourceVersion) {
    sourceVer = allVersions.find(v => v.name === sourceVersion || v.id === sourceVersion) ?? null
  }
  if (!sourceVer) {
    try {
      const pmPath = join(packDir, 'pack.mcmeta')
      const pmContent = readFileSync(pmPath, 'utf-8')
      const pm = JSON.parse(pmContent)
      const sf = pm.pack?.supported_formats
      if (sf) {
        const maxFormat = typeof sf === 'number' ? sf : sf.max_inclusive ?? sf.max ?? sf
        sourceVer = allVersions.find(v => v.data_pack_version === maxFormat) ?? null
      }
    } catch { }
  }

  const portingForward = targetVer.data_version > (sourceVer?.data_version ?? 0)
  const actions: PortingAction[] = []
  const manualAttention: PortingPlan['manualAttention'] = []

  // Build resource index and analyze
  ...
  // Load CMD_REWRITES from fixer for plan generation
  const { CMD_REWRITES } = await import('./fixer.js')

  const svDv = sourceVer?.data_version ?? 0
  const tvDv = targetVer.data_version

  // Determine applicable command rewrites
  const applicableRewrites = CMD_REWRITES.filter(rw => {
    const sourceSinceDv = rw.sourceSince ? versionNameToDataVersion(rw.sourceSince, allVersions) : null
    const targetUntilDv = rw.targetUntil && rw.targetUntil !== '0' ? versionNameToDataVersion(rw.targetUntil, allVersions) : null
    const targetSinceDv = rw.targetSince ? versionNameToDataVersion(rw.targetSince, allVersions) : null
    if (sourceSinceDv !== null && svDv < sourceSinceDv) return false
    if (targetUntilDv !== null && tvDv > targetUntilDv) return false
    if (targetSinceDv !== null && tvDv < targetSinceDv) return false
    return true
  })

  // Find commands that need rewriting
  const funcResources = resources.filter(r => r.type === 'function')
  for (const func of funcResources) {
    const fullPath = join(packDir, func.file)
    let content: string
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      for (const rw of applicableRewrites) {
        ...
        if (rw.pattern.test(cmdLine)) {
          const newLine = cmdLine.replace(rw.pattern, rw.replacement)
          actions.push({
            file: func.file,
            line: i + 1,
            type: rw.replacement.includes('## FIXED') ? 'command_comment' : 'command_rewrite',
  ...
  // Check for features that need manual attention (no rewrite available)
  for (const func of funcResources) {
    const fullPath = join(packDir, func.file)
    let content: string
    try { content = readFileSync(fullPath, 'utf-8') } catch { continue }
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      const hasRewrite = applicableRewrites.some(rw => rw.matchRoot === root)
      const hasKnowledgeRule = FEATURE_RULES.some(rule => {
        if (rule.type !== 'command') return false
        if (rule.match !== root) return false
        const ruleMinDv = versionNameToDataVersion(rule.minVersion, allVersions)
        if (ruleMinDv === null) return false
        return svDv >= ruleMinDv && tvDv < ruleMinDv
      })

      if (hasKnowledgeRule && !hasRewrite) {
        manualAttention.push({
          file: func.file,
          description: `Command "${root}" not available in ${targetVersion}`,
          reason: `No automatic rewrite available. Needs manual replacement.`,
        })
      }
    }
  }
  ...
  for (const action of actions) {
    const affected = graph.dependedBy.get(action.file)
    if (affected && affected.size > 0) {
      cascadeEffects.push({
        trigger: action.description,
        triggerFile: action.file,
        affectedFiles: [...affected],
        description: `Rewriting ${action.file} may affect ${affected.size} dependent file(s)`,
      })
    }
  }

  // Report orphans as potential dead code
  for (const orphan of orphans) {
    manualAttention.push({
      file: orphan.file,
      description: `Orphaned ${orphan.type} "${orphan.fullPath}" — not referenced by anything`,
      reason: 'This resource may be dead code. Verify it is still needed.',
    })
  }

  const autoFixable = actions.filter(a => a.autoFixable).length
  const filesAffected = new Set(actions.map(a => a.file)).size

  return {
    sourceVersion: sourceVer?.name ?? 'unknown',
    targetVersion,
    portingForward,
    actions,
    cascadeEffects,
    ...
    summary: {
      ...
      autoFixable,
      manualRequired: manualAttention.length,
      filesAffected,
      cascadeCount: cascadeEffects.length,
    },
  }
}
